...
function applyWeekWorkDays(weekStartDate, workDays, today = startOfDay(new Date())) {
	const weekKey = dateKey(startOfWeek(weekStartDate));
	...
	const editableNextDays = workDays.filter((dayNum) => !isPastWorkDay(dayNum));
...
function initDefaultWeeks() {
	...
	const defaultWorkDays = Array.isArray(state.settings.defaultWorkDays) ? state.settings.defaultWorkDays : [2];
	for (let d = startOfWeek(start); d <= end; d = addDays(d, 7)) {
		const wk = dateKey(d);
		if (!state.weekWorkDays[wk]) {
			state.weekWorkDays[wk] = [...defaultWorkDays];
		}
		ensureWeekSlots(wk);
	}
}
...
function isWithinCustomerVisibleWindow(datePart, refDate = new Date()) {
	const d = parseDateKey(datePart);
	if (!d) return false;
	const today = startOfDay(refDate);
	const slotDay = startOfDay(d);
	const diffDays = Math.floor((slotDay.getTime() - today.getTime()) / (24 * 60 * 60 * 1000));
	return diffDays >= 0 && diffDays < CUSTOMER_FUTURE_DAYS;
}
...
function buildStateForSocket(socket) {
	...
	const customerRangeEnd = addDays(today, Math.max(0, CUSTOMER_FUTURE_DAYS - 1));
	...
	Object.values(state.slots).forEach((slot) => {
		if (slot.hiddenByMaster) return;
		if (!isWithinCustomerVisibleWindow(slot.datePart, now)) return;
...
io.on("connection", (socket) => {
	...
	socket.on("executor:updateWeekWorkDays", ({ weekStart, workDays }) => {
		...
		const safeDays = Array.isArray(workDays)
			? [...new Set(workDays.map(Number).filter((d) => Number.isInteger(d) && d >= 0 && d <= 6))].sort((a, b) => a - b)
			: [];

		state.settings.defaultWorkDays = [...safeDays];

		const futureLimit = addDays(today, MASTER_FUTURE_DAYS);
		for (let d = new Date(weekStartDate); d <= futureLimit; d = addDays(d, 7)) {
			applyWeekWorkDays(d, safeDays, today);
		}
	...
	socket.on("executor:updateWorkingHours", ({ startHour, endHour }) => {
		...
		if (safeStart === null || safeEnd === null) {
			socket.emit("error:message", "Укажите часы в формате 0..23 или HH:00.");
			return;
		}
...
app.post("/login", async (req, res) => {
    ...
    const expectedEmail = String(masterEmail || "").trim().toLowerCase();
    const expectedPassword = String(masterPassword || "");

    let ok = false;

    if (email === expectedEmail) {
        // Если задан bcrypt-хэш в переменной окружения MASTER_PASSWORD_HASH — проверяем по нему
        if (process.env.MASTER_PASSWORD_HASH) {
            try {
                ok = await bcrypt.compare(password, process.env.MASTER_PASSWORD_HASH);
            } catch {
                ok = false;
            }
        } else {
            // Иначе обычная проверка по MASTER_PASSWORD
    ...
    if (String(req.body.remember || "").toLowerCase() === "on" || String(req.body.remember || "") === "1") {
